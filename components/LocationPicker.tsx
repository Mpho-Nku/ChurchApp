...
export default function LocationPicker({ onSelect }: LocationPickerProps) {
  ...
  // 🔍 Auto-fetch results as user types
  useEffect(() => {
    ...
    }, 400); // 400 ms debounce like Instagram
  ...
  const handleMapClick = (lat: number, lon: number) => {
    const loc = { name: `Lat: ${lat.toFixed(4)}, Lon: ${lon.toFixed(4)}`, lat, lon };
  ...
  return (
    <div className="relative">
      ...
      <div className="flex items-center mb-2 relative">
        ...
        {loading && <Loader2 className="animate-spin text-gray-400 ml-2" size={16} />}
      </div>
